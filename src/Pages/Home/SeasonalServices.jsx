      ...
      <AnimatePresence mode="wait">
        ...
        >
          {sesonalServices.map((product) => (
            <div
              key={product.id}
              className="relative bg-white shadow-md rounded-xl  flex flex-col justify-start items-center text-center transition duration-300 hover:shadow-lg"
            >
              <img
                src={product.contentUrl || product.image}
                alt={product.title || product.description}
                className="w-full h-32 object-fit-cover mb-4"
              />
